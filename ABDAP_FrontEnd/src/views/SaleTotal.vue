...
const initSalesTrendChart = async () => {
  ...
  const option = {
    title: { text: '销量趋势', textStyle: { fontSize: 14, color: '#333' } },
    tooltip: {
      trigger: 'axis',
    ...
    xAxis: { type: 'category', data: data.map((item: any) => item.date) },
    yAxis: {
      type: 'value',
      name: '销量(辆)',
...
const initSalesAmountChart = async () => {
  ...
  if (salesAmountChartInstance) salesAmountChartInstance.dispose()
  salesAmountChartInstance = echarts.init(salesAmountChart.value)
  const option = {
    title: { text: '销售额变化', textStyle: { fontSize: 14, color: '#333' } },
    tooltip: {
      trigger: 'axis',
      formatter: (params: any) => {
        const data = params[0]
...
const initTopModelsChart = async () => {
  ...
  if (topModelsChartInstance) topModelsChartInstance.dispose()
  topModelsChartInstance = echarts.init(topModelsChart.value)
  const option = {
    title: { text: '车型销量排行', textStyle: { fontSize: 14, color: '#333' } },
    tooltip: {
      trigger: 'axis',
    ...
    grid: { left: '25%', right: '4%', bottom: '3%', top: '15%', containLabel: true },
  }
...
const initRegionSalesChart = async () => {
  ...
  if (regionSalesChartInstance) regionSalesChartInstance.dispose()
  regionSalesChartInstance = echarts.init(regionSalesChart.value)
  const option = {
    title: { text: '地区销量分布', textStyle: { fontSize: 14, color: '#333' } },
    tooltip: {
      trigger: 'axis',
      axisPointer: { type: 'shadow' },
      formatter: (params: any) => {
...
const navigateToTopModels = (date?: string) => {
  ...
  router.push({ name: 'TopCarModelList', query })
}
...
const navigateToHeatMap = (region?: string) => {
  ...
  router.push({ name: 'CarPurchasesHeatMap', query })
}
